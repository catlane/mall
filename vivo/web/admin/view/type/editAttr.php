...
<div class="member_show" >
    <form action="">
        <div class="form-group has-success">
            ...
            <input type="text" class="form-control" id="type_attr_name" name="type_attr_name" value="{{$data['type_attr_name']}}">
        </div>
        <div class="form-group has-error">
            <label class="control-label" for="type_attr_value">类型属性值</label>
            <input type="text" class="form-control" id="type_attr_value" name="type_attr_value" value="{{$data['type_attr_value']}}" placeholder="多个属性值请用 | 分开  ">
        </div>
        <div class="form-group has-warning">
            ...
            <select name="type_attr_type" id="type_attr_type" class="form-control">
                <option value="1" <if value="$data['type_attr_type']==1">selected</if> >规格</option>
                <option value="0" <if value="$data['type_attr_type']==0">selected</if> >属性</option>
            </select>
        </div>
        <input type="hidden" name="type_attr_id" value="{{$data['type_attr_id']}}">
        <input type="hidden" name="type_type_id" value="{{$data['type_type_id']}}">
...
<script>
    $('.submit').click(function(){
        ...
        $.post("{{u('type/editAttr')}}",data,function (res) {
            if(res.valid){
