                ...
                <div id="sample-table_wrapper" class="dataTables_wrapper no-footer">
                    <table class="table table-border table-bordered table-bg table-hover table-sort dataTable no-footer" id="sample-table" role="grid" aria-describedby="sample-table_info">
                        ...
                        <tr class="text-c" role="row">
                            ...
                            <?php foreach ((array)$spec as $key=>$value){?>
                                <th width="60" class="sorting_disabled" rowspan="1" colspan="1" aria-label="商品名称" style="width: 102px;"><?php echo $key?></th>
                            <?php }?>
                        ...
                        <tbody>
                            <?php foreach ((array)$oldData as $key=>$value){?>
                                <tr role="row" class="odd">
                                        ...
                                        <td class="sorting_1"><?php echo $value['goods_lists_id']?></td>
                                        <?php foreach ((array)$value['goods_lists_allid'] as $k=>$v){?>
                                            <td><?php echo $v?></td>
                                        <?php }?>
                                        <td><?php echo $value['goods_lists_stock']?></td>
                                        <td><?php echo $value['goods_lists_num']?></td>
                                        <td class="td-manage">
                                            <a title="删除" href="javascript:;" onclick="member_del(this,'<?php echo $value["goods_lists_id"]?>')" class="btn btn-xs btn-warning"><i class="icon-trash  bigger-120"></i></a>
                                        </td>
                                </tr>
                            <?php }?>
                                <tr role="row" class="even">
                                    <form action="<?php echo u('goodslist/add')?>" class="form" method="post">
                                    <td><label><input type="checkbox" class="ace" value=""><span class="lbl"></span></label></td>
                                    <td class="sorting_1">添加货品</td>
                                    <?php foreach ((array)$spec as $key=>$value){?>
                                        <td>
                                            <select name="goods_lists_allid[<?php echo $key?>][]" id="">
                                    ...
                                    <td class="td-manage">
                                        ...
                                        <input type="hidden" name="commodity_commodity_id" value="<?php echo q('get.commodity_id','0','intval')?>">
        ...
        <script>
            $(function () {
                $('.form').submit(function () {
                    ...
                    $.post("<?php echo u('goodslist/add')?>",data,function (res) {
                        if(res.valid){
                            layer.alert('添加成功',{
                                title: '提示框',
                                icon:1,
                                yes:function () {
                                    location.href = "<?php echo u('goodsList/index',array('commodity_id'=>q('get.commodity_id','0','intval')))?>";
                                }
                            });
                            layer.close();
                            return true;
        ...
        <script>
            ...
            function member_del(obj,id){
                layer.confirm('确认要删除吗？',function(index){
                    $.post("<?php echo u('goodsList/del')?>",{'goods_lists_id':id},function(){
                    },'json')
		...
		<script>
			$(function () {
				...
				$.each($("#nav_list li").find("ul.submenu").find("li a"),function () {
					if(""){
						if($(this).parent("li").parent("ul").siblings("a").find("span").html().indexOf("") > -1){
