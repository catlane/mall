...
<script type="text/javascript">
    ...
    <link rel="stylesheet" href="<?php echo __ROOT__?>/web/admin/view/assets/css/ace.min.css" rel="stylesheet">
    <!--    这是为了分类那里的文件夹所用的-->
    <link rel="stylesheet" href="<?php echo __ROOT__?>/web/admin/view/Widget/zTree/css/zTreeStyle/zTreeStyle.css" rel="stylesheet">
...
<!--    这是为了分类那里的文件夹所用的-->
    <script src="<?php echo __ROOT__?>/web/admin/view/Widget/zTree/js/jquery.ztree.all-3.5.min.js"></script>
    ...
    <div class="clearfix">
        <div class="admin_info_style">
            <div class="border clearfix">
               <span class="l_f">
                    <a href="<?php echo u('category/add')?>" title="添加类型" class="btn btn-warning Order_form"><i class="icon-plus"></i>添加分类</a>
               </span>
                <span class="r_f">共：<b><?php echo count($data)?></b>个分类</span>
            </div>
            <div>
                        ...
                        <table class="table table-border table-bordered table-bg table-hover table-sort dataTable no-footer" id="sample-table" role="grid" aria-describedby="sample-table_info">
                            ...
                            <?php foreach ((array)$data as $k=>$v){?>
                                <tr role="row" class="odd">
                                    ...
                                    <td class="sorting_1"><?php echo ($v['category_id'])?></td>
                                    <td>
                                        <a href="<?php echo u('category/category_info',array('category_id'=>$v['category_id']))?>"><?php echo ($v['category_name'])?></a>
                                    </td>
                                    <td class="sorting_1"><?php echo ($v['category_sort'])?></td>
                                    <td class="sorting_1">
                                        <?php if($v['category_pid'] == 0){?>
                
                                            顶级分类
                                        <?php }else{?>
                                            <?php echo Db::table('category')->where('category_id',$v['category_pid'])->pluck('category_name')?>
                                        
               <?php }?>
                                    </td>
                                    <td class="sorting_1"><?php echo ($v['type_name'])?></td>
                                    <td class="td-manage">
                                        <a title="添加子集分类" onclick="member_edit('添加子集分类','<?php echo u('category/addSon',array('category_id'=>$v['category_id']))?>','<?php echo ($v['category_id'])?>','','510')" href="javascript:;" class="btn btn-xs btn-success"><i class="icon-edit bigger-120"></i></a>
                                        <a title="编辑" onclick="member_edit('编辑','<?php echo u('category/edit',array('category_id'=>$v['category_id']))?>','<?php echo ($v['category_id'])?>','','510')" href="javascript:;" class="btn btn-xs btn-info"><i class="icon-edit bigger-120"></i></a>
...
<script>
    ...
    function member_del(obj,id){
        layer.confirm('确认要删除吗？',function(index){
            $.post("<?php echo u('category/del')?>",{'category_id':id},function(res){
                if(res.valid == 0){
