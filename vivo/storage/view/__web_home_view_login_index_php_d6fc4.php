...
<html>
	<head>
		...
		<link rel="stylesheet" href="<?php echo __ROOT__?>/resource/hdjs/home/hdjs/hdjs.css">
		...
		<script src="<?php echo __ROOT__?>/resource/hdjs/home/hdjs/hdjs.min.js"></script>
	</head>
	<body>
							...
							<script>
								$(function () {
									$('.dl').submit(function () {
										...
										$.post("<?php echo u('login/login')?>",data,function (res) {
...
<!--								</li>-->
								...
								<li class="check">
									...
									<a href="<?php echo u('login/webprotocol')?>" target="_blank" class="xieyi">《vivo服务协议》</a>
								</li>
								<li class="registered-btn">
									<input type="submit" />
								</li>
						...
						<input type='hidden' name='__TOKEN__' value='5c4f7d97fcbb48d52c7e758bab625b20'/></form>
							<!--用手机注册end-->
							<!--用邮箱注册start-->
						<form action="<?php echo u('login/registered')?>" method="post" class="reg" id="EMAIL">
			...
			<script>
				$(function () {
					...
					$('.code').keyup(function () {
						var This = $(this);
						var code = $(this).val();
						$.post("<?php echo u('login/yzCode')?>",{"code":code},function (res) {
							if(res){
					...
					$('input[name=member_user_phone]').blur(function () {
						var This = $(this);
						var phone = $(this).val();
						$.post("<?php echo u('login/yzPhone')?>",{"phone":phone},function (res) {
							if(res){
								if(This.parent().find('.correct').length){
									This.parent().find('.correct').remove();
									This.parent().find('.error').remove();
					...
					$('input[name=member_user_email]').blur(function () {
						var This = $(this);
						var email = $(this).val();
						$.post("<?php echo u('login/yzEmail')?>",{"email":email},function (res) {
							if(res){
								if(This.parent().find('.correct').length){
									This.parent().find('.correct').remove();
									This.parent().find('.error').remove();
					...
					$('.reg').submit(function () {
						var data = $(this).serialize();
						$.post("<?php echo u('login/registered')?>",data,function (res) {
							if(res.valid){
