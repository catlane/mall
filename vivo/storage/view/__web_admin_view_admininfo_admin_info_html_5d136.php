...
<div class="clearfix">
 <div class="admin_info_style">
   <div class="admin_modify_style" id="Personal">
       ...
       <form action="" method="post" class="info">
           <div class="xinxi">
               <div class="form-group"><label class="col-sm-3 control-label no-padding-right" for="username">用户名： </label>
                   <div class="col-sm-9">
                       <input type="text" name="admin_user_username" id="username" value="<?php echo $user_info['admin_user_username']?>" class="col-xs-7 text_info" disabled="disabled">
               ...
               <div class="form-group"><label class="col-sm-3 control-label no-padding-right" for="age">年龄： </label>
                   <div class="col-sm-9"><input type="text" name="admin_user_age" id="age" value="<?php echo $user_info['admin_user_age']?>" class="col-xs-7 text_info" disabled="disabled"></div>
               </div>
               <div class="form-group"><label class="col-sm-3 control-label no-padding-right" for="phone">移动电话： </label>
                   <div class="col-sm-9"><input type="text" name="admin_user_phone" id="phone" value="<?php echo $user_info['admin_user_phone']?>" class="col-xs-7 text_info" disabled="disabled"></div>
               </div>
               <div class="form-group"><label class="col-sm-3 control-label no-padding-right" for="email">电子邮箱： </label>
                   <div class="col-sm-9"><input type="text" name="admin_user_email" id="email" value="<?php echo $user_info['admin_user_email']?>" class="col-xs-7 text_info" disabled="disabled"></div>
               </div>
               <div class="form-group"><label class="col-sm-3 control-label no-padding-right" for="QQ">QQ： </label>
                   <div class="col-sm-9"><input type="text" name="admin_user_qq" id="QQ" value="<?php echo $user_info['admin_user_qq']?>" class="col-xs-7 text_info" disabled="disabled"> </div>
               </div>
               <div class="form-group"><label class="col-sm-3 control-label no-padding-right">权限： </label>
                   <div class="col-sm-9" > <span><?php echo $user_info["role_name"]?></span></div>
               </div>
               <div class="form-group"><label class="col-sm-3 control-label no-padding-right">注册时间： </label>
                   <div class="col-sm-9" > <span><?php echo date('Y-m-d h:i:s',$user_info['admin_user_regTime'])?></span></div>
               </div>
           ...
           <input type="hidden" name="admin_user_id" value="<?php echo $user_info['admin_user_id']?>">
    ...
    <tbody>
      <?php foreach ((array)$loginLogs as $key=>$value){?>
          <tr>
              <td><label><input type="checkbox" class="ace" value="<?php echo $value['loginLog_id']?>"><span class="lbl"></span></label></td>
              <td><?php echo $value['loginLog_id']?></td>
              <td><?php echo $user_info["role_name"]?><!--这里暂时没有写，因为后台没有做到这里了，到时候，再加把--></td>
              <td>登录成功!</td>
              <td><?php echo $value['loginLog_address']?></td>
              <td><?php echo $value['admin_user_account']?></td>
              <td><?php echo $value['loginLog_IP']?></td>
              <td><?php echo date('Y-m-d H:i:s',$value['loginLog_time'])?></td>
          </tr>
      <?php }?>
    </tbody>
  </table>
...
function save_info(){
     ...
     $(".xinxi input[type$='text']").each(function(n){
		  ...
		  if(num>0){  return false;}	 	
              ...
              $.post("<?php echo u('admininfo/changeInfo')?>",data,function (res) {
                    if(res.valid){
                        layer.alert('修改成功！',{
                            title: '提示框',
                            icon:1,
		    ...
		    if(!$("#c_mew_pas").val || $("#c_mew_pas").val() != $("#Nes_pas").val() )
        {
         ...
         if($("#c_mew_pas").val()==$("#password").val()){
             ...
             return false;
              ...
              $.post("<?php echo u('admininfo/changePassword')?>",data,function (res) {
                    if(res.valid){
		...
		<script>
			$(function () {
				...
				$.each($("#nav_list li").find("ul.submenu").find("li a"),function () {
					if("管理员管理"){
						if($(this).parent("li").parent("ul").siblings("a").find("span").html().indexOf("管理员管理") > -1){
