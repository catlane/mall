                            ...
                            <tr role="row" class="odd">
                                ...
                                <td>
                                    <?php foreach ((array)$v['lists'] as $key=>$value){?>
                                        <a href="<?php echo u('home/item/item',array('commodity_id'=>$value['commodity_id']))?>" target="_blank">
...
<!--                                    订单状态0:未付款 1:待收货 2未评价3:已取消-->
                                    <span class="label label-success radius">
                                        <?php echo $v['orderForm_status']?>
                                    </span>
                                </td>
                                <td class="td-manage">
                                    <?php if($v['orderForm_status'] == '未发货'){?>
                
                                        <a title="发货" onclick="Delivery_stop(this,'10061','<?php echo $v['orderForm_id']?>')" href="javascript:;" class="btn btn-xs btn-success"><i class="icon-truck"></i></a>
                                    
               <?php }?>
                                    <a title="订单详细" href="javascript:;" class="btn btn-xs btn-info"><i class="icon-list bigger-120"></i></a>
                                    <a title="删除" href="javascript:;" orderForm_status="<?php echo $v['orderForm_status']?>" onclick="member_del(this,'<?php echo $v['orderForm_id']?>')" class="btn btn-xs btn-warning"><i class="icon-trash  bigger-120"></i></a>
                                </td>
                            </tr>
                        <?php }?>
                        </tbody>
            ...
            function member_del(obj,orderForm_id){
                ...
                if($(obj).attr("orderForm_status") == "已完成" || $(obj).attr("orderForm_status") == "已取消"){
                    layer.confirm('确认要删除吗？',function(index){
                        $.post("<?php echo u('orderForm/del')?>",{'orderForm_id':orderForm_id},function(res){
                            if(res){
